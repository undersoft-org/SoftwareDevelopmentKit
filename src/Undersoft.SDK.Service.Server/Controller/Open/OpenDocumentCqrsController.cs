...
[OpenData]
[RemoteResult]
public abstract class OpenDocumentCqrsController<TKey, TEntry, TReport, TEntity, TDto, TService>
...
{
    protected OpenDocumentCqrsController() { }

    public OpenDocumentCqrsController(IServicer servicer) : base(servicer) { }
    ...
    {
        return _servicer.Report(new GetDocument<TReport, TEntity, TDto>()).Result.Result;
    }
    ...
    public override SingleResult<TDto> Get([FromRoute] TKey key)
    {
        return new SingleResult<TDto>(
           _servicer
               .Report(
                   new FindDocument<TReport, TEntity, TDto>(
    ...
    public override async Task<IActionResult> Patch([FromRoute] TKey key, [FromBody] TDto dto)
    {
        ...
        var result = await _servicer.Send(
            new ChangeDocument<TEntry, TEntity, TDto>(_publishMode, dto, _predicate)
        );
