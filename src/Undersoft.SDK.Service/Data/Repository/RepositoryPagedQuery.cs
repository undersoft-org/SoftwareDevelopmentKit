...
public partial class Repository<TEntity>
{
        ...
        where TDto : class
    {
        ...
        if (typeof(TEntity) != typeof(TDto))
            ...
            return Task.FromResult(
                (IQueryable<TDto>)this[skip, take, this[predicate, sortTerms, expanders]]
            );
    }
        ...
        where TDto : class
    {
        ...
        if (typeof(TEntity) != typeof(TDto))
            return MapQueryAsync<TDto>(
                this[skip, take, this[predicate, sortTerms, expanders]].AsNoTracking()
            );
        else
            return Task.FromResult(
                (IQueryable<TDto>)
    ...
    public virtual async IAsyncEnumerable<TDto> FilterNoTrackedStreamAsync<TDto>(
        int skip,
        int take,
        Expression<Func<TEntity, bool>> predicate,
        SortExpression<TEntity> sortTerms,
        ...
        where TDto : class
    {
        if (typeof(TEntity) != typeof(TDto))
            await foreach (
                var item in MapToAsync<TDto>(
                    this[skip, take, predicate, sortTerms, expanders].AsNoTracking()
                )
            )
                yield return item;
        else
            foreach (
                var item in this[skip, take, this[predicate, sortTerms, expanders]].AsNoTracking()
            )
    ...
    public virtual IAsyncEnumerable<TDto> FilterStreamAsync<TDto>(
      ...
      where TDto : class
    {
        return MapToAsync<TDto>(this[skip, take, this[predicate, sortTerms, expanders]]);
    }
    ...
    public virtual Task<IQueryable<TDto>> GetNoTrackedQueryAsync<TDto>(
        int skip,
        int take,
        SortExpression<TEntity> sortTerms,
        params Expression<Func<TEntity, object>>[] expanders
        ...
        where TDto : class
    {
        if (typeof(TEntity) != typeof(TDto))
            ...
            return Task.FromResult(
                (IQueryable<TDto>)this[skip, take, this[sortTerms, expanders]].AsNoTracking()
            );
    }
